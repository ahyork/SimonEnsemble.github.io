...
the Markovian property. This property states that any given state holds all of
the information from previous states in order to make a prediction. This is
because a Markov chain is memoryless and only "remembers" the current state. As
soon as it transitions it forgets where it has been. This means that a Markov
chain must be designed in such a way that every state holds all of the
...
weather, so that is all we need to track in our model.
...
predict the next state and fails to satisfy the Markovian property if the
weather from the previoous week is relevant for predicting tomorrow's weather.
We can modify our system to track all of these transitioning states, but that
...
represents the graph from figure 1. Each column corresponds to a different
current state: 1 is sunny, 2 is cloudy, 3 is rainy, and 4 is snowy. The sum
across each column is equal to 1 because at any given state it MUST transition
to another state (this can just be back to itself).
